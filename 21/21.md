...
Decompressing until it's not a zlib file will give you a bzip file. You can tell that it's a zlib file with the file header `BZh91AY` as we've experienced before back in level8.
...
Hey it looks like a zlib file again!. The zlib file header we saw before `b'x\x9c` is at the start of the reversed byte-string. 
...
`b'sgol ruoy ta kool'` is the result. I can't read it lol, we should probably reverse it as well. 
